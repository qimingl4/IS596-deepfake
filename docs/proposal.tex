...
\textbf{Ethical and Legal Gap.} Victims of deepfake abuse---particularly NCII victims---need more than binary classification. They require forensic reports with traceable evidence chains (timestamped frames, cryptographic hashes, phoneme-level analysis) that can support platform moderation appeals, legal proceedings, and digital advocacy \citep{abercrombie2024taxonomy}.
...
Early deepfake detection relied on visual artifact analysis---detecting blending inconsistencies, unnatural eye blinking, or spectral artifacts in face-swapped images \citep{li2019exposing, afchar2018mesonet}. While effective against first-generation face swaps, these methods are brittle against newer synthesis techniques.

Audio-visual approaches represent a more robust detection paradigm. SyncNet \citep{chung2017out} pioneered audio-visual synchronization analysis. \citet{haliassos2021lips} proposed \emph{Lips Don't Lie}, demonstrating that irregular mouth movements detectable through lipreading models serve as reliable deepfake indicators.
...
\textbf{Wav2Vec 2.0} \citep{baevski2020wav2vec} demonstrated that self-supervised pre-training on unlabeled speech data learns rich acoustic representations. The model's hidden states encode articulatory configurations without explicit supervision \citep{cho2023evidence}, making Wav2Vec2 an effective audio encoder whose representations implicitly encode the physical speech production process.

\subsection{Explainable AI for Media Forensics}

A critical limitation of existing deepfake detectors is their opacity. CNNs and transformer-based classifiers produce confidence scores without explaining \emph{what} makes a video suspicious \citep{arrieta2020explainable}. Recent work has explored attention visualization \citep{wodajo2021deepfake}, temporal saliency maps \citep{haliassos2022leveraging}, and natural language explanations generated by multimodal LLMs \citep{jia2024chatgpt}. Our work extends this line by using an LLM as an active reasoning agent that synthesizes multiple detection signals into structured forensic analyses grounded in physical evidence.

\subsection{Cognitive Intervention and Dual Process Theory}

\citet{kahneman2011thinking} distinguishes between fast, automatic ``System~1'' processing and slow, deliberative ``System~2'' reasoning. Media consumption overwhelmingly engages System~1, making users vulnerable to deepfakes \citep{pennycook2019lazy}. Research demonstrates that accuracy nudges significantly reduce misinformation sharing \citep{pennycook2021shifting}, and ``prebunking'' approaches build resilience \citep{roozenbeek2022combat}. Our system operationalizes these insights by providing structured forensic evidence that transforms passive consumption into active evaluation.

\subsection{Harm Taxonomy and Ethical Frameworks}

\citet{abercrombie2024taxonomy} developed a comprehensive taxonomy of harms from AI-generated media, distinguishing content-level, distribution-level, and systemic harms. Our system incorporates this taxonomy through automated harm categorization (political manipulation, NCII, financial fraud, general synthetic media) with tailored recommended actions. The system is designed as a transparency tool providing evidence for human judgment, mitigating the dual-use concern of detection systems being weaponized for censorship \citep{paris2019deepfakes}.
...
The audio encoder extracts articulatory representations using \textbf{Wav2Vec2} (\texttt{facebook/wav2vec2-base-960h}) \citep{baevski2020wav2vec}, a self-supervised speech model pre-trained on 960 hours of unlabeled English speech. Audio is extracted from video using FFmpeg, resampled to 16\,kHz mono, and processed to produce \textbf{768-dimensional frame-level embeddings} at 50\,fps (one frame per 20\,ms). Optionally, the \textbf{Montreal Forced Aligner} (MFA) \citep{mcauliffe2017montreal} produces phoneme-level temporal alignment, enabling explicit detection of bilabial phoneme mismatches.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{DFDC} \citep{dolhansky2020deepfake}: The DeepFake Detection Challenge dataset containing over 100,000 video clips.
    \item \textbf{ASVspoof 2019} \citep{todisco2019asvspoof}: For evaluating audio-only detection capabilities.
\end{itemize}
...
\bibitem[Abercrombie et~al., 2024]{abercrombie2024taxonomy}
Abercrombie, G., Curry, A.~C., Dinkar, T., and Rieser, V. (2024).
\newblock A taxonomy of harms from {AI}-generated media.
\newblock In {\em Proceedings of the ACM Conference on Fairness, Accountability, and Transparency (FAccT)}.

\bibitem[Afchar et~al., 2018]{afchar2018mesonet}
Afchar, D., Nozick, V., Yamagishi, J., and Echizen, I. (2018).
\newblock {MesoNet}: A compact facial video forgery detection network.
\newblock In {\em IEEE International Workshop on Information Forensics and Security (WIFS)}.

\bibitem[Ajder et~al., 2019]{ajder2019state}
Ajder, H., Patrini, G., Cavalli, F., and Cullen, L. (2019).
\newblock The state of deepfakes: Landscape, threats, and impact.
\newblock {\em Deeptrace Labs Report}.
...
\newblock In {\em Advances in Neural Information Processing Systems (NeurIPS)}, volume~33.

\bibitem[Chen et~al., 2024]{chen2024deepfake}
Chen, Y. et~al. (2024).
\newblock Deepfake-enabled financial fraud: Emerging threats and countermeasures.
\newblock {\em IEEE Security \& Privacy}.
...
\bibitem[Cho et~al., 2023]{cho2023evidence}
Cho, S. et~al. (2023).
\newblock Evidence of articulatory information in self-supervised speech representations.
\newblock In {\em Proceedings of Interspeech}.

\bibitem[Chung and Zisserman, 2017]{chung2017out}
...
\newblock In {\em IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)}.

\bibitem[Haliassos et~al., 2022]{haliassos2022leveraging}
Haliassos, A., Mira, R., Petridis, S., and Pantic, M. (2022).
\newblock Leveraging real talking faces via self-supervision for robust forgery detection.
\newblock In {\em IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)}.

\bibitem[Jia et~al., 2024]{jia2024chatgpt}
Jia, S. et~al. (2024).
...
\bibitem[McAuliffe et~al., 2017]{mcauliffe2017montreal}
...
\bibitem[Paris and Donovan, 2019]{paris2019deepfakes}
Paris, B. and Donovan, J. (2019).
\newblock Deepfakes and cheap fakes.
\newblock {\em Data \& Society Report}.

\bibitem[Pennycook and Rand, 2019]{pennycook2019lazy}
Pennycook, G. and Rand, D.~G. (2019).
\newblock Lazy, not biased: Susceptibility to partisan fake news is better explained by lack of reasoning than by motivated reasoning.
\newblock {\em Cognition}, 188:39--50.
...
\bibitem[Roozenbeek and van~der Linden, 2022]{roozenbeek2022combat}
Roozenbeek, J. and van~der Linden, S. (2022).
\newblock How to combat health misinformation: A psychological approach.
\newblock {\em American Journal of Tropical Medicine and Hygiene}, 107(2):14--17.
...
\bibitem[Wodajo and Atnafu, 2021]{wodajo2021deepfake}
Wodajo, D. and Atnafu, S. (2021).
\newblock Deepfake video detection using convolutional vision transformer.
\newblock {\em arXiv preprint arXiv:2102.11126}.
